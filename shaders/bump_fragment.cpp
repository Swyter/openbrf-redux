...
#endif
    float lightAmbient = gl_LightSource[0].ambient.r;
    float lightDiffuse = gl_LightSource[0].diffuse.r;
    gl_FragColor.rgb = tex.rgb*color.rgb*(vec3(diffuse*lightDiffuse + lightAmbient )
